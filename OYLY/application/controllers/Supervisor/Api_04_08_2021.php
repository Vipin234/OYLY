...
public function staff_update_post()
{
  ...
  if($row2 > 0)
  {
    ...
    for($i=0;$i<count($explode);$i++){
      ...
      // }
      // else{
      //   $result1 =$this->Supervisor->staff_registration($data);
      // }
        $result1 =$this->Supervisor->update_staff_profile($data);
  }
  if(!empty($mobile_no)){
      ...
      if(!empty($admin_id)&&!empty($from_date)&&!empty($to_date))
      {
          ...
          if(!empty($result))
          {
            ...
            foreach($result as $value)
            {
              ...
              $final_array[$create_date]['total_amount']+=$value['total_order_amount'];
              $final_array[$create_date]['discount']+=$value['discount'];
              $final_array[$create_date]['discount_amount']+=$value['discount_amount'];
              $final_array[$create_date]['net_sales']+=($value['total_order_amount']-$value['discount_amount']);
              ...
              $i++;
               ...
               $aray=array('status'=>'1','data'=>$final_array,'message'=>'success');
               $this->response($aray, 200);
          }else
          {
            $aray=array('status'=>'0','message'=>'failed');
